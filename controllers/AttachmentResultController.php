...
use app\models\AttachmentResult;
...
class AttachmentResultController extends Controller
{
    /**
     * @inheritDoc
     */
    ...
    public function actionCreate($experiment_id)
    {
        ...
        if ($this->request->isPost && $model->load($this->request->post())) {
            ...
            if ($model->file) {
                ...
                $model->file_name = "$model->experiment_id-result";
    ...
    public function actionDelete($experiment_id)
    {
        ...
        try {
            if ($model->delete() && $model->deleteFile()) {
                ...
                $transaction->rollBack();
                Yii::$app->session->addFlash('error', 'C\'è un problema: non è stato possibile rimuovere il documento. Riprova o contatta l\'assistenza tecnica.');
            }
        } catch (\Throwable $t) {
            $transaction->rollBack();
            throw $t;
    ...
    {
        if (($model = AttachmentResult::findOne(['experiment_id' => $experiment_id])) !== null) {
            return $model;
        }
